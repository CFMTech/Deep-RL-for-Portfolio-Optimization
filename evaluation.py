...
def test_models(path_weights, env, fc1_units=16, fc2_units=8, random_state=1024,
                n_episodes=100):
    """
    Description
    ---------------
...
def plot_min_max(env, models_keys, scores_episodes):
    ...
    for i, model_key in enumerate(models_keys):
        ...
        plt.subplot(2, 3, i+1)
        plt.plot(range(len(signal_min)), signal_min, label='min %.2f' %
                 score_episode[random_state_min])
        plt.plot(range(len(signal_max)), signal_max, label='max %.2f' %
                 score_episode[random_state_max])
        plt.legend()
...
def plot_function(path_weights, env, models_keys, fc1_units=16, fc2_units=8, low=-1,
                  high=1, step=0.01, pi=0.5, psi=.3, lambd=.5, thresh=2, clip=True):
    """
    Description
    ---------------
    ...
    signal_ones_neg = torch.tensor(
        np.vstack((range_values, -pi*np.ones(len(range_values)))).T, dtype=torch.float)
    if env.squared_risk:
        optimal1 = optimal_f_vec(signal_ones_neg[:, 0].numpy(
        ), -pi, lambd=lambd, psi=psi, cost=env.cost)
        optimal2 = optimal_f_vec(signal_zeros[:, 0].numpy(), 0, lambd=lambd, psi=psi,
                                 cost=env.cost)
        optimal3 = optimal_f_vec(signal_ones_pos[:, 0].numpy(),
                                 pi, lambd=lambd, psi=psi, cost=env.cost)

    else:
        optimal1 = optimal_max_pos_vec(signal_ones_neg[:, 0].numpy(), -pi, thresh,
                                       env.max_pos)
        optimal2 = optimal_max_pos_vec(signal_zeros[:, 0].numpy(), 0, thresh, env.max_pos)
        optimal3 = optimal_max_pos_vec(signal_ones_pos[:, 0].numpy(), pi, thresh,
                                       env.max_pos)
    ...
    for i, model_key in enumerate(models_keys):
        ...
        with torch.no_grad():
            if clip:
                model1 = np.clip(model(signal_ones_neg)[
                                 :, 0].data.numpy(), -env.max_pos + pi, env.max_pos + pi)
                model2 = np.clip(model(signal_zeros)[:, 0].data.numpy(), -env.max_pos,
                                 env.max_pos)
                model3 = np.clip(model(signal_ones_pos)[
                                 :, 0].data.numpy(), -env.max_pos - pi, env.max_pos - pi)
